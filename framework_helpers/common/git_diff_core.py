"""
Framework-neutral git diff extraction.
...
Framework adapters correlate the resulting GitDiffInfo with their own architecture.
"""
...
class GitDiffCore:
    ...
    def get_diff_info(self) -> GitDiffInfo:
        ...
        try:
            ...
            if has_changes:
                return self._extract_working_tree_diff(status_lines)

            commit_count = self._get_commit_count()
            if commit_count >= 2:
                return self._extract_last_two_commits_diff()
            elif commit_count == 1:
                return self._extract_single_commit_diff()
            return GitDiffInfo(
                is_git_repo=True,
                comparison_mode="none",
